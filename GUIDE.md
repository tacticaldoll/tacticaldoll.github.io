...
- **系列宣告資格 (Series Eligibility) [CRITICAL]**：`is_series` 的唯一判準是 session 目錄中 `guide*.md` 的實體存在（見 [init-handoff.task.schema.yaml](.agent/schemas/init-handoff.task.schema.yaml)）。單篇主報告 session 依規省略導讀檔，即為 Standalone，其 `series-map.md` **嚴禁**宣告 `series`——沒有任何腳本解析 series-map 的 `series`（權威欄位是 handoff 的 `metadata.series`），宣告只會製造「內部地圖有系列、已發布貼文沒有」的分歧。由 `audit_kb.py` 物理攔截，TOML 與散文兩種形狀皆攔。
...
- **獨立傳播力 (Independent Propagability) [CRITICAL]**：所有發布貼文——無論是否屬於系列——都必須能被單獨閱讀而不產生錯誤理解。系列是「同病因的不同切片」，不是「連續性解方的章節」。具體判準：
    - **零跨篇依賴**：正文不得出現「如前所述」「詳見系列第 X 篇」「接續上篇」等指向其他貼文的語句，也不得包含指向其他貼文的站內連結。
    - **術語自錨定**：所有專業術語在該篇首次出現時必須完成定義錨定，不假設讀者具備系列背景。
    - **首段自定位**：導言必須讓首次接觸的讀者在單一閱讀單元內掌握本篇的問題、對象與可帶走的價值，不依賴系列脈絡。
    - **結尾無預告**：結尾收束於本篇自身的結論，不得以「下篇將探討」等方式製造對未發布內容的依賴。
- **參考文獻混合式規範 (Hybrid Citation) [NEW]**：所有蒸餾報告與發布貼文的參考文獻採用「論文式引用 + 穩定連結」的混合式。具體判準：
    - **引用本體**：保留完整論文式條目（作者、年份、篇名、出處、卷期頁碼，可得範圍內）。
    - **穩定連結**：每條必須附一個可點擊的穩定識別符，優先順序為 DOI > arXiv > JSTOR stable URL > 出版社頁面；禁止附易失效的搜尋結果頁或短網址。
    - **書籍豁免**：專書（含經典著作與教科書）無自然穩定識別符，豁免連結要求，但必須附 **ISBN**；無 ISBN 可考的早期著作（如 19 世紀文獻）標註初版資訊即可。
    - **官方文件與判例**：判決書、調查報告、法規文件附官方機構的 stable URL；無官方線上來源者標記「無官方線上來源」。
    - **誠實標記**：無法定位確切出處的條目，必須在條目內明確標記，**嚴禁**偽造篇名、卷期或 DOI。統一使用三種標記形態：
        - 出處查無實據：`（DOI 待查證；[期刊目次](URL)）`
        - 官方來源不存在：`（無官方線上來源；出版者）`
        - 連結曾存在但失效：`（連結失效，待補；書目識別符）`
    - **生效範圍**：本規範自 2026-09-22 起適用於新產出內容；既有內容的回溯補連結為獨立批次作業，需逐條查證後以原子化替換進行，不適用自動化批次腳本（見貼文不可變協議）。
