...
- 一個鍵是一個概念。同概念的第二種正當譯法列為 `aliases`（錯定在同鍵、不改寫原文）；概念不同則另立鍵。晉升時同鍵異 zh 會被擋下，不得以覆蓋解決。
- 技術標籤顯示其術語在該篇正文中的形式，正文沒有就不得存在（`TagAnchorer.body_form`）。發布與重新錯定共用同一套採收與上限邏輯，所以兩者對同一篇得到同一組標籤；規則曾只寫在任務清單，44 個標籤因此在無人察覺下漂離。
