...
- 術語錨定對中文不設正字法字界，因此短鍵會匹配到更長詞的內部（`量化` 落在「輕量化」、`技術債` 落在「技術債務」、`導讀` 落在「誤導讀者」）。字界是人寫下的資料而非推導：落在較長詞內的命中列入該術語的 `not_within`，由 `Lexicon.find_hits` 單一定義，錨定、標籤採收、領域偵測與稽核共用；同字異義的獨立命中（「無法量化」不是 Quantization）沒有字界可言，由貼文 front matter 的 `term_exclude` 記錄。兩者都是人的判讀結果，機制只負責保存與執行；**降級或收錄判定仍必須檢視命中位置**，發布時由 `term_review` 閘門要求逐條判讀。
- 一個鍵是一個概念。同概念的第二種正當譯法列為 `aliases`（錨定在同鍵、不改寫原文）；概念不同則另立鍵。晉升時同鍵異 zh 會被擋下，不得以覆蓋解決。
- 技術標籤顯示其術語在該篇正文中的形式，正文沒有就不得存在（`TagAnchorer.body_form`）。發布與重新錨定共用同一套採收與上限邏輯，所以兩者對同一篇得到同一組標籤；規則曾只寫在任務清單，44 個標籤因此在無人察覺下漂離。
- 錨定不得落在作者的粗體範圍內。作者把「動詞＋術語（註記）」整段加粗時，那是作者的單位；在裡面錨定會切斷粗體並把作者的註記交給另一個術語。
...
- 逐篇稽核以錨定程式自身的契約檢查錨點：正文等於目前術語庫對它的重新錨定結果，且每個被錨定的鍵恰有一個定義框。稽核不得另立一套與錨定程式輸出格式相左的規則。
