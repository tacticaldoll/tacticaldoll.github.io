...
- **標籤與發布同一套規則**:genre 與領域標籤來自發布期的 `scope` 與領域宣告,本流程只刷新其顯示值。技術標籤則直接呼叫發布組裝程式的採收與上限邏輯:既有標籤作為策展集排在前,正文命中的術語補滿空位;每個標籤都必須顯示其術語在正文中的形式(`TagAnchorer.body_form`),正文沒有就刪除。因此重新發布與重新錯定得到同一組標籤,術語庫成長時舊貼文的標籤也跟著收斂。
- **貼文的判讀紀錄**:front matter 的 `term_exclude` 列出該篇不錯定的鍵(同字異義),本流程移除其內文錯點、不再重貼,並刪除同鍵標籤。術語的 `not_within`(中文字界)與 `aliases`(譯法並存)同樣在此生效。舊版錯定切斷作者粗體的形狀(`**動詞**術語**（註記） <!-- term -->**`)會還原為作者原文。
