...
「存在一個產生相同預測的替代權重分佈」這個事實，需要仔細界定。若那個替代分佈是**被訓練出來的**——也就是你另外最佳化一組注意力參數，使它既遠離原分佈又保持預測——那麼你得到的是一個**不同的模型**。這個新模型的存在，並不說明原模型實際使用的那組權重毫無意義。[Wiegreffe 與 Pinter，《Attention is not not Explanation》](https://aclanthology.org/D19-1002/)
