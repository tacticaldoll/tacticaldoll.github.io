...
$$
Q_\差異=\差異\operatorname{round}(w/\差異),
$$

在不截斷且取最近格點時，單一權重誤差滿足 $|Q_\差異-w|\leq\差異/2$。但輸出誤差還受輸入尺度、層間放大與非線性影響。因果鏈是：表示格點變粗，權重或啟動值產生誤差，logit 邊際縮小或改號，個體決策翻轉，而平均值可能因正負翻轉互抵而不變。
...
| 蒸餾 <!-- term:Distill --> | 學生的函數與容量 | 教師、硬標籤學生與軟目標學生 | 教師忠實度、校準 <!-- term:Calibration -->、資料外差異 |
...
驗證契約：資料使用凍結測試集、低邊際集及罕見切片；切分與校準 <!-- term:Calibration -->集互斥；seed 為 0–9；指標含平均與最差切片分數、翻轉率、ECE、延遲、記憶體和能耗；控制前後處理、批次、硬體與測量工具；操弄位元數、稀疏度或學生容量；若行為差異在未壓縮重跑中同樣出現，則反駁壓縮歸因；在達到資源目標且所有風險指標低於預設容差，或 Pareto 前緣不再改善時停止。
