...
| 「介入一直在變，所以無法評估」 | 混淆「無法點識別」與「無法設界」 | 放棄量化，改以敘事判斷 | 部分識別 <!-- term:PartialIdentification -->給界限；界限同樣排除觀察，同樣可證偽 |
...
錯誤的作法是因為「介入一直在變」而放棄量化評估，改以敘事判斷。這在形式上等於把不可證偽性 <!-- term:Falsifiability -->當成一種謙遜。
