...
控制變因是真實正確率、樣本數與 seed；操弄變因是試驗次數。預期最佳觀察值隨挑選機會增加而上升。程式不表示所有高分虛假，也未模擬模型間真**差異**（Delta） <!-- term:Delta -->；它只隔離「多看再挑」的機制。
...
Benchmark 與 Demo 是能力的測量介面，不是能力本身。可信市場訊號必須保留資料、提示、指標、挑選程序與部署差異 <!-- term:Delta -->。分數可以簡化決策；只有當簡化留下可反駁的外推契約，它才不是把有限證據包裝成無限能力。