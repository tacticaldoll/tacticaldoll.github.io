...
```
   客觀物理現實分佈 p_0(x) (蘊含豐富長尾與邊界條件)
             │
             ▼ (第一代採樣訓練)
       模型 p_θ0(x)
             │
             ▼ (以合成數據自我投餵)
       模型 p_θ1(x) ──> [ 早期坍塌: 長尾分佈開始滅絕 ]
             │
             ▼ (進一步遞歸投餵)
       模型 p_θ2(x) ──> [ 晚期坍塌: 方差發散、模式崩潰 ]
             │
             ▼ (不可逆熱力學死寂)
       Dirac Delta 奇異分佈 δ(x - μ) [ 輸出退化為無意義亂碼 ]
```
...
```
┌────────────────────────────────────────────────────────────────────────┐
│             Slopsquatting 幻覺依賴攻擊與供應鏈投毒拓樸                 │
├────────────────────────────────────────────────────────────────────────┤
│                                                                        │
│    [ 工程師提出需求 ] ──> [ 大語言模型 (LLM) ]                         │
│                                  │                                     │
│                                  ▼ (產生機率性幻覺依賴)                │
│    [ 程式碼生成: import "cloud-vault-auth" ] (此包原本不存在)          │
│                                  │                                     │
│    ┌─────────────────────────────┴─────────────────────────────┐       │
│    │                                                           │       │
│    ▼ (攻擊者利用反向工程提前感知)                               ▼ (工程師操作) │
│ [ 黑客在 PyPI 搶註該包 ]                                [ 執行 pip install ]  │
│ [ 植入木馬後門與資訊竊取代碼 ]                                   │       │
│    │                                                           │       │
│    └─────────────────────────────┬─────────────────────────────┘       │
│                                  │                                     │
│                                  ▼                                     │
│    [ 惡意代碼靜默混入企業核心軟體 ] ──> [ 密鑰被盜 / 生產伺服器被黑 ]   │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```
