...
帳本層的兩個觸發器則把成本物理降成不可繞過的狀態不變式 <!-- term:Invariant -->。解除註解後實測的中止訊息分別是 `不變式違反：modify 必須指向既有單元` 與 `不變式違反：導入成本必須與當前系統規模 N 成正比`。第二條特別值得注意：它禁止的是「把導入成本 <!-- term:IntegrationCost -->以固定值登帳」這個記帳習慣，而這正是讓 $O(N)$ 在報表上看起來像 $O(1)$ 的最常見手法。
