...
這裡的「主體、客體、能力、邊界」是最小詞彙。主體是會讀取並行動的人或 agent。客體是被讀取、修改或驗證的程式碼、文件、規格、工具與記憶。能力是載入、搜尋、生成、修改、執行與驗證。邊界則決定主體在何種情境下能對哪些客體使用哪些能力。治理文件的任務，就是讓這些邊界足夠清楚，讓 agent 不需要用猜測填補制度空白。
...
這也說明為什麼**後設資料**（Metadata） <!-- term:Metadata -->是漂移高風險區。文件主體被重命名時，front matter description、範例、註解與索引常常被漏掉。agent 讀取這些殘留文字後，可能把已退場的術語重新帶回輸出。治理若只看正文，就會留下足以污染後續工作的邊角材料。
...
當這些原則成立時，AI agent 不需要靠猜測理解專案制度。它可以從薄入口找到正確客體，沿著路由讀取相關規則，遇到衝突時依驗證優先序決策，產生新知識時再按價值歸位。這才是 AI 協作治理的實際目標：不是讓模型一次讀完所有東西，而是讓它在每一步都能找到足夠正確的權威。