...
驗證契約如下：資料固定為成對快照並保留個體識別；切分採預先凍結的參考集與最新部署切片；seed 至少 5 個；指標含整體分數、個體翻轉率與切片差；控制模型、資料、推論、測量時每次只換一軸；觀察量是 $\差異 S$ 及其區間；若固定三軸後差異消失，即反駁被替換軸的歸因；當區間窄於預先定義的最小實務差異，或資源上限用盡時停止。
